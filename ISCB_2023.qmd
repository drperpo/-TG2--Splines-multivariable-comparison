...
author: 
  ...
  -affiliation: "Medical University of Vienna,  GSK"
...
pt_fp <- predict(fit_mfp, data=dat1, type="contrasts", se=TRUE, terms="BMI", ref=list(BMI=30))
...
plot_ci <- function(x, fit, se=NULL, lower=NULL, upper=NULL, linecol="black", fillcol="lightgray", ylim=c(-7,7)){
  if(!is.null(se)){
    lower <- fit - 1.96*se
    upper <- fit + 1.96*se
  }
  
  dat <- data.frame(BMI=x, fit, lower, upper)
  dat<- dat[order(x),]
  
  p <- ggplot(dat, aes(x=BMI, y=fit))+ geom_line(col=linecol) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = fillcol, alpha = 0.5) +ylim(ylim)
  return(p)
}
...
ndat <- dat1
ndat$Pregnancies <- mean(dat1$Pregnancies)
ndat$Glucose <- mean(dat1$Glucose)
ndat$BloodPressure <- mean(dat1$BloodPressure)
ndat$SkinThickness <- mean(dat1$SkinThickness)
ndat$Insulin <- mean(dat1$Insulin)
ndat$DiabetesPedigreeFunction <- mean(dat1$DiabetesPedigreeFunction)
ndat$Age <- mean(dat1$Age)


termfit_fp <- predict(fit_mfp, newdata=ndat, type="terms", se=TRUE)
termfit_tp <- predict(fit_tp_mgcv, newdata=dat1, type="terms", se=TRUE, seWithMean=TRUE)



termfit_rms <- predict(fit_rms, newdata=ndat, type="lp", se.fit=TRUE)
...
print(plot_ci(x=dat1$BMI, fit=termfit_rms$linear.predictors, se=termfit_rms$se.fit))


## try to overlay!
fpp <- plot_ci(x=termfit_fp$BMI[,"variable"], fit=termfit_fp$BMI[,"value"], lower=termfit_fp$BMI[,"lower"], upper=termfit_fp$BMI[,"upper"], linecol = "black", fillcol="lightskyblue")

rmsp <- plot_ci(x=dat1$BMI, fit=termfit_rms$linear.predictors, se=termfit_rms$se.fit, linecol="saddlebrown", fillcol="tan2")


tpp <- plot_ci(x=dat1$BMI, fit=termfit_tp$fit[,"s(BMI)"], se=termfit_tp$se.fit[,"s(BMI)"], linecol="red", fillcol="thistle2")




library(gridExtra)
grid.arrange(fpp, rmsp, tpp, ncol=3)
```
